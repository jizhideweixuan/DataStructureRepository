...
public:
    ...
    void clear() {// Delete all nodes in chain.
        while (chain<T>::firstNode != NULL) {// delete chain<T>::firstNode
...
template<class T>
void extendedChain<T>::erase(int theIndex) {// Delete the element whose index is theIndex.
...
template<class T>
void extendedChain<T>::insert(int theIndex, const T &theElement) {// Insert theElement so that its index is theIndex.
    if (theIndex < 0 || theIndex > chain<T>::listSize) {// invalid index
        ostringstream s;
...
template<class T>
void extendedChain<T>::push_back(const T &theElement) {// Insert theElement at the end of the chain.
